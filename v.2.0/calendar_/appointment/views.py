...
def getApointmentRequest(request):
    ...
    time_slots_to_fix=Time_slot.objects.filter(user=time_slot.user, date=time_slot.date, time=time_slot.time).all() # нашли все таймслоты на определенную дату по конкретному специалисту
    for item in time_slots_to_fix: # проходим циклом по всем найденым таймслотам и найденные таймслоты заполняем как записанные
        item.free_time = False
        item.save()
